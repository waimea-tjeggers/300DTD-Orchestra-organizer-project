...
- Access to data is restricted via user accounts
...
A suitable level of complexity for a Level 3 project might involve the following functionality:

- **Ordering**: Accessing a catalogue of products, compiling an order for a user
- **Storage**: Storing data of some kind that is categorised, on a per-user basis
- **Scoring**: Processing data input for a sport / event, with an admin account
- **Stock Management**: Access to information about large collections of objects, with an admin
- **Communication**: Allowing conversations between registered and logged-in users
- **Connections**: Connecting registered users via some sort of profile data
